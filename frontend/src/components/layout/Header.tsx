...
export function Header({ chatbotId, chatbotName }: HeaderProps) {
  const [showUserMenu, setShowUserMenu] = useState(false);

  return (
    <header className="sticky top-0 z-30 h-16 bg-white/80 backdrop-blur-xl border-b border-white/20">
      <div className="flex items-center justify-between h-full px-6">
        ...
        <div className="flex items-center gap-2">
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="text-[#D1D1D6]">/</span>
            <span className="text-[#1D1D1F] font-medium">Analytics</span>
          </nav>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-3 p-1.5 rounded-xl hover:bg-[#F5F5F7] transition-colors"
            >
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#5B5EFF] to-[#8B7FFF] flex items-center justify-center">
                <span className="text-sm font-medium text-white">JD</span>
              </div>
              <svg
                className={cn('w-4 h-4 text-[#86868B] transition-transform', showUserMenu && 'rotate-180')}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
